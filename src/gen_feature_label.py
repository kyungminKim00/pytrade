...
class FeatureLabel:
    def __init__(
        ...
        env_dict=env_dict,
        gen_expected_rtn=False,  # 학습시에만 필요
    ):
        # # missing values
        ...
        # data 생성
        if gen_expected_rtn:
            df = self.calculate_expected_rtn(data_cls, env_dict)
    ...
    def calculate_expected_rtn(self, data_cls, env_dict):
        print("calculate_expected_rtn")

        prediction_candle = pd.date_range(
            start=data_cls.index[0],
            end=data_cls.index[-1],
            freq=f"{env_dict['prediction_candle']}min",
        )
        # dummy 제거
        data_cls = data_cls.loc[data_cls.index[0] : prediction_candle[-1]]

        expected_rtn = {}
        for row in data_cls.itertuples(index=True):
            e_idx = np.argwhere(row.Index <= prediction_candle).squeeze()[0]
            s_date, e_date = row.Index, prediction_candle[e_idx]
            expected_rtn.update(
                {row.Index: data_cls.loc[s_date:e_date]["y_rtn_close"].sum()}
            )

        # expected_rtn 을 data_cls 에 추가
        new_df = pd.DataFrame(
            list(expected_rtn.items()), columns=["datetime", "expected_rtn"]
        )
        new_df.set_index("datetime", inplace=True)
        df = data_cls.append(new_df)
        return df
