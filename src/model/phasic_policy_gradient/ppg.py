...
from base_constants import AuxMemory, device
...
class PPG:
    ...
    def load(self):
        if not os.path.exists("./model/phasic_policy_gradient/ppg.pt"):
            return

        data = torch.load("./model/phasic_policy_gradient/ppg.pt")
    ...
    def learn(self, memories, aux_memories, next_state):
        ...
        def to_torch_tensor(t):
            return torch.stack(t).to(device).detach()
    ...
    def learn_aux(self, aux_memories):
        ...
        for epoch in range(self.epochs_aux):
            for states, old_action_probs, rewards, old_values in tqdm(
                ...
                # paper says,
                # it is important to train the value network extra during the auxiliary phase
                values = self.critic(states)
                value_loss = clipped_value_loss(
                    values, rewards, old_values, self.value_clip
                )
